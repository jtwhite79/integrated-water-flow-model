...
! ******************************************************************
  ...
  SUBROUTINE LWUseZoneBudRawFile_New(Logger,IsForInquiry,cFileName,TimeStep,NTIME,cVersion,lElemFlowToSubregions,Flags,AppGrid,ZBudFile,iStat)
    ...
    Header%cDSSFParts = [CHARACTER(LEN=13) :: 'AG_AREA'         ,&
                         'AG_POTNL_CUAW'   ,&
                         'AG_SUP_REQ'      ,&    
                         'AG_PUMPING'      ,&
                         'AG_DELIVERY'     ,&
  ...
  SUBROUTINE RootZoneZoneBudRawFile_New(Logger,IsForInquiry,cFileName,TimeStep,NTIME,cVersion,lElemFlowToSubregions,Flags,AppGrid,ZBudFile,iStat)
    ...
    Header%cDSSFParts = [CHARACTER(LEN=16) :: 'AG_AREA'           ,&
                         'AG_POT_ET'         ,&   
                         'AG_PRECIP'         ,&   
                         'AG_RUNOFF'         ,&   
                         'AG_PRM_H2O'        ,&
  ...
  SUBROUTINE RootZoneBudRawFile_New(IsForInquiry,cProjectNameForDSS,cFileName,TimeStep,NTIME,NRegion,RegionArea,cRegionNames,cDescriptor,cVersion,RawFile,iStat,Logger)
    ...
    ASSOCIATE (pLocation => OutputData%Locations(1))
      ...
      ASSOCIATE (pColumnHeaders => pLocation%cColumnHeaders           , &
        ...
        Text                = ArrangeText(TRIM(UnitT),17)
        Text1               = '('//TRIM(f_cAreaUnitMarker)//')'
        pColumnHeaders(:,1) = [CHARACTER(LEN=100) :: '                 ','              ','               ','                ','               ','       Prime   ','   Inflow as   ','         Reused','          Net  ','     Beginning ',' Net Gain from ','               ','        Other  ','         Actual','               ','        Ending ','               ','              ','               ','                ','               ','       Prime   ','   Inflow as   ','         Reused','          Net  ','     Beginning ',' Net Gain from ','               ','        Other  ','         Actual','               ','        Ending ','               ','              ','               ','                ','   Inflow as   ','               ','     Beginning ',' Net Gain from ','               ','        Other  ','         Actual','               ','        Ending ','               ']
    ...
    ELSEIF (iZBudgetType .EQ. f_iZBudgetType_RootZone) THEN
        SELECT CASE (iLUType)
            CASE (f_iLandUse_Ag , f_iLandUse_NonPondedAg , f_iLandUse_Rice , f_iLandUse_Refuge)
                ...
                cFlowNames = [CHARACTER(LEN=24) :: 'Change in Storage' , 'Gain from Land Expansion' , 'Infiltration' , 'Other Inflow' , 'ET' , 'Percolation']
